...
RSpec.describe Components::Repository do
  ...
  describe "#all" do
    it "returns all components" do
      ...
      expect(result.map(&:id)).to include(component1.id, component2.id)
      expect(result.map(&:name)).to include("Handlebar", "Saddle")
    end
  end
  ...
  describe "#all_by" do
    it "returns components filtered by the given filters" do
      ...
      expect(result.map(&:id)).to include(component1.id)
      expect(result.map(&:id)).to_not include(component2.id)
    end
  end
