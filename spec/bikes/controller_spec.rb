...
RSpec.describe Bikes::Controller do
  ...
  describe "#index" do
    it "returns the list of bikes" do
      allow(Bikes::Bike).to receive(:all).and_return([{id: 1, name: "Mountain Bike"}])
  ...
  describe "#create" do
    it "creates a new bike" do
      allow(Bikes::Bike).to receive(:create).and_return([{id: 1, name: "Mountain Bike"}])
      ...
      expect(Bikes::Bike).to have_received(:create).with({"name" => "Mountain Bike"})
    end
  end

  describe "#read" do
    it "returns the bike with the given id if it exists" do
      allow(Bikes::Bike).to receive(:find_by).and_return({id: 1, name: "Mountain Bike"})
  ...
  describe "#update" do
    it "updates the bike with the given id if it exists" do
      allow(Bikes::Bike).to receive(:find_by).and_return({id: 1, name: "Mountain Bike"})
      allow(Bikes::Bike).to receive(:update)
  ...
  describe "#delete" do
    it "deletes the bike with the given id if it exists" do
      ...
      allow(Bikes::Bike).to receive(:find_by).and_return(bike)
    ...
    it "returns 404 Not Found if the bike does not exist" do
      allow(Bikes::Bike).to receive(:find_by).and_return(nil)
